#!/bin/bash
set -e
apt='env -u LD_PRELOAD -u TMPDIR proot -l -0 -r . -w /usr/bin ./env PATH=/usr/bin:/bin apt'
case $(uname -m) in
        arm) ARCH=armel;;
        aarch64) ARCH=arm64;;
        i686) ARCH=i386;;
...
$apt install mawk -y
echo "alias debian='LD_PRELOAD= proot -l -0 -b tmp:/dev/shm -b /dev -b /proc -r . -w /root /bin/bash --rcfile .bashrc'" >> ~/.bashrc
rm "$0"
